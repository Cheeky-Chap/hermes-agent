...
├── agents/                   # Hermes·Commander·Coding 정책
├── hermes_plugin/            # Discord ↔ 호스트 브로커 플러그인
├── workers/                  # Codex/OpenCode 작업 위임 유틸리티
├── deploy/                   # 비밀값 없는 cron 예시
...
- Commander의 주제별 위임 정책과 Coding Agent의 작업 분류 정책
- Codex 우선·OpenCode fallback worker 실행 래퍼
...
Commander와 Coding 정책은 이 저장소에 포함되지만, Stocks와 TFT 정책 및
구현은 각 프로젝트 저장소에서 관리하여 동일 파일을 중복 보관하지 않습니다.
